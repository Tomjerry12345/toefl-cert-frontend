...
import {
  LayoutDashboard, FilePlus, FileStack,
  ShieldCheck, XCircle, GraduationCap, LogOut, Wallet
} from 'lucide-react'
...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 text-white flex flex-col z-50"
      ...
      <div className="px-6 py-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-sm text-white leading-tight">TOEFL Cert</p>
      ...
      {user?.walletAddress && (
        <div className="px-4 py-3 mx-3 mt-4 bg-white/5 rounded-xl border border-white/10">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xs text-blue-400 font-medium">Wallet Terhubung</p>
          </div>
          <p className="text-xs font-mono text-blue-200">{shortAddress(user.walletAddress)}</p>
        </div>
      )}
      ...
      <div className="px-6 py-4 border-t border-white/10">
        ...
        <p className="text-xs font-semibold text-blue-200">Ethereum Sepolia + Merkle Tree</p>
      </div>
    </aside>
  )
}
