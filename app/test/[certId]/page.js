    ...
    const verify = async () => {
      try {
        ...
        if (err.response?.status === 404) {
          ...
          setResult({
            verified: false,
            message: "Gagal verifikasi, coba lagi",
          });
        }
      } finally {
        setLoading(false);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-1">
            Mengecek di Blockchain Ethereum Sepolia
          </p>
        </div>
      </div>
    );
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-6">
          <p className="text-gray-500 text-sm">
            Sistem Verifikasi Sertifikat TOEFL
          </p>
          <p className="text-gray-400 text-xs">
            Berbasis Blockchain Ethereum & Merkle Tree
          </p>
        </div>
        ...
        >
          ...
          <p
            className={`text-sm ${result?.verified ? "text-emerald-600" : "text-red-600"}`}
          >
            {result?.verified
              ? "Terverifikasi di Blockchain Ethereum Sepolia"
              : result?.message ||
                "Sertifikat tidak ditemukan atau telah dicabut"}
          </p>
        </div>
        ...
        {result?.data && (
          <div className="bg-white rounded-2xl shadow-card p-6 border border-gray-100">
            ...
            <div className="space-y-3">
              {[
                ...
                },
                {
                  icon: Calendar,
                  label: "Tanggal Tes",
                  value: formatDate(result.data.testDate),
                },
                {
                  icon: Calendar,
                  label: "Berlaku Sampai",
                  value: formatDate(result.data.expiryDate),
                },
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              ...
              <div
                className={`p-2 rounded-lg text-center text-xs font-semibold ${
                  result.data.blockchainVerification
                    ? "bg-emerald-50 text-emerald-700"
                    : "bg-red-50 text-red-700"
                }`}
              >
